# Workload generator for HA testing in Ceph/Rook/ODF
...
## What is the ODF load generator tool?
...
When performing High Availability and resilience tests Ceph/ODF we need to generate load/IOs from the clients to see how it will be affected by the failure of different services/components in the Ceph/ODF stack.

## What workload can we create with the ODF load generator tool?
...
    - Perform a stat operation in the metadata file, this will talk directly with the Ceph MDS servers.
  - If any I/O operation (data and metadata) elapsed time is greater than `LOG_SECONDS` second/s, we log the operation.
...
We need to be logged in to our OCP and in the namespace where we want to deploy our load generator pods
...
Finally, deploy the ODF load generator tool installing the chat with helm:

```
cd ocs-ha-tests/
helm install odf-load generator helm
```
